...
@section('title', 'Edit Raqi Availability')

@section('content_header')
    <h1>Edit Raqi Availability</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('superadmin.raqi-availability.update', $availability) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select name="practitioner_id" id="practitioner_id" class="form-control @error('practitioner_id') is-invalid @enderror" required>
                                ...
                                @foreach($practitioners as $practitioner)
                                    <option value="{{ $practitioner->id }}" {{ old('practitioner_id', $availability->practitioner_id) == $practitioner->id ? 'selected' : '' }}>
                                        {{ $practitioner->name }} 
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" value="{{ $availability->availability_date ? $availability->availability_date->format('M d, Y (l)') : 'No date set' }}" readonly>
                            <small class="text-muted">Date cannot be changed after creation</small>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="time" name="start_time" id="start_time" 
                                   ...
                                   value="{{ old('start_time', $availability->start_time ? $availability->start_time->format('H:i') : '') }}" required>
                            @error('start_time')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="time" name="end_time" id="end_time" 
                                   ...
                                   value="{{ old('end_time', $availability->end_time ? $availability->end_time->format('H:i') : '') }}" required>
                            @error('end_time')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="is_available" name="is_available" value="1" {{ old('is_available', $availability->is_available) ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <textarea name="notes" id="notes" rows="3" 
                              ...
                              placeholder="Any additional notes about this availability">{{ old('notes', $availability->notes) }}</textarea>
                    @error('notes')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
